...
@interface SendViewController ()<ZoomImageViewDelegate,CLLocationManagerDelegate,FaceViewDelegate,UITextViewDelegate>
...
//    1.NSLog(@"%@",self.mm_drawerController); 无法获取
//    2.通过appDelegate
    
    NSLog(@"%@",self.view.window.rootViewController);

     MMDrawerController *drawer = (MMDrawerController *)self.view.window.rootViewController;
...
- (void)sendStatus{
    ...
    if (_location) {
        //设置经纬度
        [params setValue:[NSString stringWithFormat:@"%f",_location.coordinate.latitude] forKey:@"lat"];
        [params setValue:[NSString stringWithFormat:@"%f",_location.coordinate.longitude] forKey:@"long"];
    }
    ...
    //带图片
    if (_sendImg) {
        urlSting = @"statuses/upload.json";
        
        //图片转为data
        NSData *imgData = UIImageJPEGRepresentation(_sendImg, 1);
        //图片不能大于5M
        if (imgData.length > 3 * 1024 * 1024) {
            
            //压缩图片
            imgData = UIImageJPEGRepresentation(_sendImg, .5);
        }
        
        //设置图片数据
        fileData = @{@"pic":imgData};
        
        //不带图片
    }else {
        
        urlSting = @"statuses/update.json";
    }
    //发送请求
    NSURLSessionDataTask *task = [DataService requestWithURL:urlSting httpMethod:@"POST" params:params fileData:[fileData mutableCopy] success:^(id result) {
    ...
    //如果是发送图片,则显示进度条
    if (fileData) {
        [self showTipWindow:(NSURLSessionUploadTask *)task];
    }
